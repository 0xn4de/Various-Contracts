...
contract MarketTest is Test {
    ...
    function setUp() public {
        market = NFTMarket(vyperDeployer.deployContract("Jan08_NFTMarket"));
    ...
    function testCreateTrade() public {
        ...
        INFTMarket(address(market)).createTrade(address(nft), bobtokensToSell, address(nft2), 1, block.timestamp+1);
    }
    function testCreateAndAcceptTradeNFTToNFT() public {
        ...
        uint256 tradeId = INFTMarket(address(market)).createTrade(address(nft), bobtokensToSell, address(nft2), 1, block.timestamp+1);
    ...
    function testCreateAndAcceptTradeNFTToMultipleNFTs() public {
        ...
        uint256 tradeId = INFTMarket(address(market)).createTrade(address(nft), bobtokensToSell, address(nft2), 2, block.timestamp+1);
