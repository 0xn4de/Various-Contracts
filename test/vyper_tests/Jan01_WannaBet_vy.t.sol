...
contract WannaBetTest is Test {
    ...
    function setUp() public {
        // vyper deployment doesnt like forks, not sure why, so will have to act differently
        // since no forks, will also have to use "mock" price feed
        //vm.createSelectFork(vm.rpcUrl('mainnet'), 18800000); // ETH Price 2254.51
        priceFeed = new AggregatorV3Interface();
        betContract = WannaBet(vyperDeployer.deployContract("Jan01_WannaBet", abi.encode(address(priceFeed))));
