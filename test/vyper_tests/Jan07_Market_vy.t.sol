...
interface IMarket {
    // due to a weird issue in my vyper install with it not recognizing uint32, uint80 etc
    // IMarket(address(market)).createTrade is replaced with IMarket(address(market)).createTrade
    function createTrade(address,uint256,address,uint256,uint256) external payable returns (uint256);
}
...
contract MarketTest is Test {
    ...
    function setUp() public {
        market = Market(vyperDeployer.deployContract("Jan07_Market"));
    ...
    function testCreateTrade() public {
        ...
        IMarket(address(market)).createTrade(address(token), 100*1e18, address(token2), 50*1e18, uint32(block.timestamp)+1);
    }
    function testCreateAndAcceptTradeTokenToToken() public {
        vm.prank(bob);
        // bob creates trade: 100 tokens for 50 token2s
        uint256 tradeId = IMarket(address(market)).createTrade(address(token), 100*1e18, address(token2), 50*1e18, uint32(block.timestamp)+1);
    ...
    function testCreateAndAcceptTradeETHToETH() public {
        ...
        uint256 tradeId = IMarket(address(market)).createTrade{value: 0.5 ether}(address(0), 0.5 ether, address(0), 0.45 ether, uint32(block.timestamp)+1);
    ...
    function testCreateAndAcceptTradeETHToToken() public {
        ...
        uint256 tradeId = IMarket(address(market)).createTrade{value: 0.5 ether}(address(0), 0.5 ether, address(token2), 100*1e18, uint32(block.timestamp)+1);
        assertEq(token2.balanceOf(alice), 1000*1e18);
    ...
    function testCreateAndAcceptTradeTokenToEth() public {
        ...
        uint256 tradeId = IMarket(address(market)).createTrade(address(token), 100*1e18, address(0), 0.5 ether, uint32(block.timestamp)+1);
    ...
    function testCreateTradeRevertOnNoValue() public {
        ...
        vm.expectRevert("No ETH sent");
        IMarket(address(market)).createTrade(address(0), 0.5 ether, address(0), 0.45 ether, uint32(block.timestamp)+1);
