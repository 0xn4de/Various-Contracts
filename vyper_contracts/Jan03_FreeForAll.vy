...
TOTAL_SUPPLY: constant(uint256) = 10000
startTime: public(uint256)
currentTokenId: public(uint256)
...
def _addTokenTo(_to: address, _tokenId: uint256):
    ...
    if _to.is_contract: # check if `_to` is a contract address
        returnValue: bytes4 = ERC721Receiver(_to).onERC721Received(msg.sender, self, _tokenId, b"")
...
def _transferFrom(_from: address, _to: address, _tokenId: uint256, _sender: address):
    ...
    if block.timestamp < self.startTime + 3600 and block.timestamp > self.startTime:
        ...
        # Check requirements
        assert self._isApprovedOrOwner(_sender, _tokenId), "NOT_AUTHORIZED"
        # Throws if `_to` is the zero address
        assert _to != empty(address), "INVALID_RECIPIENT"
...
def mintTo(_to: address) -> bool:
    ...
    # Throws if `_to` is zero address
    assert _to != empty(address), "INVALID_RECIPIENT"
    assert self.currentTokenId < TOTAL_SUPPLY, "OVER_SUPPLY"
    self.currentTokenId += 1
    # Add NFT. Throws if `_tokenId` is owned by someone
    self._addTokenTo(_to, self.currentTokenId)
    log Transfer(empty(address), _to, self.currentTokenId)
    return True
...
def tokenURI(tokenId: uint256) -> String[132]:
    assert self.idToOwner[tokenId] != empty(address), "NOT_MINTED"
