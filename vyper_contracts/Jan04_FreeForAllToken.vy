...
# lifted from https://github.com/vyperlang/vyper/blob/master/examples/tokens/ERC20.vy
...
@external
def __init__(_name: String[32], _symbol: String[32], _decimals: uint8, _supply: uint256, mintTo: address):
    init_supply: uint256 = _supply * 10 ** convert(_decimals, uint256)
    self.name = _name
    self.symbol = _symbol
    self.decimals = _decimals
    self.balanceOf[mintTo] = init_supply
    self.totalSupply = init_supply
    self.startTime = block.timestamp + 86400
    log Transfer(empty(address), mintTo, init_supply)
...
def transferFrom(_from : address, _to : address, _value : uint256) -> bool:
    ...
    """
    # NOTE: vyper does not allow underflows
    #      so subtractions would revert on insufficient allowance/balance
